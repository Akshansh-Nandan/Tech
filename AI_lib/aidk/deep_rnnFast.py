...
import numpy as np
import pandas as pd
from aidk.token_manager import tokenize, untokenize
...
class FastDeepRNN:
    ...
    def _build_model(self):
        layers = []
        # Input reshape layer: (batch_size, timesteps=1, features=num_in)
        layers.append(tf.keras.layers.InputLayer(shape=(1, self.num_in)))

        # Recurrent hidden layers
        for i, (h, act) in enumerate(zip(self.num_h, self.layer_activations[1:-1])):
            return_sequences = i < len(self.num_h) - 1
            layers.append(tf.keras.layers.SimpleRNN(
                h,
    ...
    def calculate(self, input_prompt):
        ...
        x = np.array([x], dtype=np.float32)       # shape: (1, num_in)
        x = x.reshape((1, 1, self.num_in))        # shape: (batch_size=1, timesteps=1, features=num_in)
        y = self.model(x, training=False)[0].numpy()
